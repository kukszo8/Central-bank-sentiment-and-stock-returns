...
with open(snakemake.input.body_file, 'r') as file:
    speeches = [loads(line) for line in file]

sentiments = []
for speech in tqdm(speeches[:3]):
    try:
        sentiment = structured_extract_from_longtext(prompt, speech, FedSpeech)
